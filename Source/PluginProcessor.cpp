...
void SimpleEQAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    this->leftChain.prepare(spec);
    this->rightChain.prepare(spec);

    auto chainSettings = getChainSettings(this->apvts);

    updatePeakFilter(chainSettings);

    auto cutCoeff = juce::dsp::FilterDesign<float>::designIIRHighpassHighOrderButterworthMethod(chainSettings.lowCutFreq, getSampleRate(), (chainSettings.lowCutSlope + 1) * 2);
...
void SimpleEQAudioProcessor::updatePeakFilter(ChainSettings& chainSettings)
{
...
void SimpleEQAudioProcessor::updateCoefficients(Coefficients& coeffToUpdate, Coefficients& replacements)
{
    *coeffToUpdate = *replacements;
}
